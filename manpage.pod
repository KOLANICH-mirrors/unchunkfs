...
B<unchunkfs> { B<-h> | <I<chunk dir>> <I<mount point>> }
...
might anticipate the problem that the snapshot of a journalling filesystem
...
create from an arbitrary read-only block device a writeable one by the
